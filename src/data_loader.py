...
def bert_batch_preprocessing(batch):
    ...
    y_table = true_table(doc_len_b, doc_couples_b)
    ...
    return np.array(y_table), np.array(doc_len_b), np.array(adj_b), \
...
def true_table(doc_len_b, doc_couples_b):
    N = max(doc_len_b)
    table = []
    for couples in doc_couples_b:
        adj = np.zeros((N, N))
